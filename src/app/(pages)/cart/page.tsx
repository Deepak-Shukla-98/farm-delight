...
function processProducts(products: Product[]) {
  ...
  products.forEach((product) => {
    const { id, name, price, photo, status } = product;
    const key = `${id}-${name}-${price}-${photo}-${status}`;
...
function Cart() {
  ...
  useEffect(() => {
    setProducts(() => processProducts(products_in_cart));
  }, [products_in_cart]);
  ...
  const decQuantity = (id: number) => {
    setProducts((prevProducts) =>
      prevProducts.map((product) => {
        if (product.id === id && !!product.quantity && product.quantity > 1) {
                ...
                <div className="space-y-2">
                  <dl className="flex items-center justify-between gap-4">
                    ...
                    <dd className="text-base font-medium text-gray-900 dark:text-white">
                      ${products.reduce((a, s) => a + s.price * s.quantity, 0)}
                    </dd>
                  </dl>
                  ...
                  <dl className="flex items-center justify-between gap-4">
                    ...
                    <dd className="text-base font-medium text-green-600">
                      -${products.reduce((a, s) => a + s.quantity, 0) * 100}
                    </dd>
                  </dl>
