"use client";
import { getOrdersById } from "@/components/services/axios";
...
function Page() {
  ...
  const getData = async () => {
    let [data] = await getOrdersById({ id: searchParams.get("id") });
    if (!!data) setOrders(data);
  };
  useEffect(() => {
    getData();
  }, []);
  const total = orders.orderItems.reduce((a: any, s: any) => a + s?.price, 0);
